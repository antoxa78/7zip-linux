...
pub fn create_panel(initial_path: &Path, show_hidden: Rc<Cell<bool>>) -> (gtk::Box, SharedPanel) {
    ...
    let filter_model = gtk::FilterListModel::new(Some(raw_store.clone()), Some(string_filter.clone()));
    ...
    let state = Rc::new(RefCell::new(PanelState {
        ...
        string_filter: string_filter.clone(),
    ...
    menu_model.append_section(None, &archive_section);
    ...
    menu_model.append_section(None, &misc_section);
...
fn update_status_bar(selection: &gtk::MultiSelection) {
    // This is a placeholder - actual update happens via the state's status_label
    let _ = selection;
}
                ...
                if let Some(dest_path) = dest_dir.path() {
                    ...
                    glib::spawn_future_local(async move {
                        for path in &paths {
                            ...
                            if let Err(e) = crate::operations::copy::copy_file(path, &dst).await {
                                crate::utils::show_error("Copy Failed", &e);
                            }
                        }
                        load_directory(&s2);
                    });
                ...
                if let Some(dest_path) = dest_dir.path() {
                    ...
                    glib::spawn_future_local(async move {
                        for path in &paths {
                            ...
                            let dst = dest_path.join(&name);
                            if let Err(e) = crate::operations::move_move::move_file(path, &dst).await {
                                crate::utils::show_error("Move Failed", &e);
                            }
                        }
                        load_directory(&s2);
                    });
...
pub fn on_activate(state: &SharedPanel, position: u32) {
    ...
    if let Some((path_str, is_dir)) = info {
            ...
            crate::archive::browse::parse_archive_path(&path)
        {
            ...
            open_archive_entry(&archive, &internal, pw);
        }
    }
}

fn open_archive_entry(archive: &Path, internal: &str, stored_password: Option<String>) {
    ...
    glib::spawn_future_local(async move {
        ...
        match crate::archive::extractor::extract_entry(
                ...
                match crate::archive::browse::prompt_for_password(&archive_name).await {
                    Some(password) => {
                        if let Err(e) = crate::archive::extractor::extract_entry(
                            &archive, &internal, &tmp, Some(&password),
                        )
                        .await
                        {
...
pub fn load_directory(state: &SharedPanel) {
    ...
    let (raw_store, show_hidden) = {
        let s = state.borrow();
        (s.raw_store.clone(), s.show_hidden.get())
    };
